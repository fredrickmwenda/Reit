...
use App\Repositories\RentAvailabilityRepository;
...
class AvailabilityService extends AbstractService
{
    ...
    public function addRentAvailability(Request $request)
    {
        $check_in = $request->post('calendar_check_in', '');
        $check_out = $request->post('calendar_check_out', '');
        $post_id = $request->post('calendar_post_id', '');
        ...
        $rentAvaiRepo = RentAvailabilityRepository::inst();

        $status = $request->post('calendar_status', 'available');
        if ($status == 'remove') {
            for ($i = $check_in; $i <= $check_out; $i = strtotime('+1 day', $i)) {
                $rentAvaiRepo->removeCalendarItem($post_id, $i);
            }
            return [
                'status' => true,
                'type' => 'success',
        ...
        $countUpdate = 0;
        for ($i = $check_in; $i <= $check_out; $i = strtotime('+1 day', $i)) {
            $availExists = $rentAvaiRepo->where([
                'post_id' => $post_id,
                'check_in' => $i
            ], true);
            if (!$availExists) {
                ...
                $rentAvaiRepo->create($data);
                $countUpdate++;
            } else {
                $updated = $rentAvaiRepo->updateByWhere([
                    'post_id' => $post_id,
                    'check_in' => $i
                ], [
                    'price' => $price,
    ...
    public function getRentAvailability(Request $request)
    {
        $results = [];
        $post_id = $request->post('post_id', '');
        $check_in = $request->post('start', '');
        ...
        if (!empty($post_id) and !empty($check_in) and !empty($check_out)) {
            $rentAvaiRepo = RentAvailabilityRepository::inst();
            $data = $rentAvaiRepo->getDataAvailabilityForCalendar($post_id, $check_in, $check_out);
