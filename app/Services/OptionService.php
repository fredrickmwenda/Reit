...
class OptionService extends AbstractService
{
    ...
    {
        // info('the request setto be saved',  $request->all());
        ...
        // Check if sandbox is off and both public key and client secret are present

        $payment_paystack_sandbox = $request->post('payment_paystack_sandbox');
        if ($payment_paystack_sandbox === 'off') {
            // Both public key and client secret should be present
            $payment_paystack_public_key = $request->post('payment_paystack_public_key');
            $payment_paystack_client_secret = $request->post('payment_paystack_client_secret');
    
            if ($payment_paystack_public_key && $payment_paystack_client_secret) {
                // Check if the public key already exists in the PaystackData table
                $existing_public_key = DB::table('paystack_data')->where('public_key', $payment_paystack_public_key)->exists();
                if (!$existing_public_key) {
                    // Store the public key and client secret in the PaystackData table
                    DB::table('paystack_data')->insert([
                        'public_key' => $payment_paystack_public_key,
                        'client_secret' => $payment_paystack_client_secret,
