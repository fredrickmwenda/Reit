...
?>
            <?php if ($wrapper) { ?>
                ...
                <?php } ?>
                <a href="javascript:void(0)" class="add-wishlist" data-id="<?php echo $post_id ?>" data-post-type="<?php echo $post_type; ?>" data-action="<?php echo url('add-wishlist'); ?>" data-toggle="tooltip" data-placement="top" title="<?php echo $title_wishlist; ?>">
                    <?php if ($in_wishlist) { ?>
...
if (!function_exists('get_the_permalink')) {
    function get_the_permalink($post_slug, $post_type = 'post')
    {
        switch ($post_type) {
            ...
            case GMZ_SERVICE_PROPERTY:
                return get_property_permalink($post_slug);
                break;
...
if (!function_exists('get_search_string')) {
    function get_search_string($service, $total_result, $params)
    {
        ...
        } elseif ($service == GMZ_SERVICE_PROPERTY) {
            if ($total_result > 0) {

                if ($total_result == 1) {
                    if (empty($params['address'])) {
                        $search_str = __('Found <b>1 Property</b>');
                    }
                    // if (empty($params['address'])) {
                    //     $search_str = sprintf(__('Found <b>1 Rent</b> from <b>%s</b> to <b>%s</b>'), $checkIn, $checkOut);
                    // }
                    // if (!empty($params['address']) && empty($params['checkIn'])) {
                    //     $search_str = sprintf(__('Found <b>1 Apartment</b> in <b>%s</b>'), urldecode($params['address']));
                    // }
                    ...
                    if (!empty($params['address'])) {
                        $search_str = sprintf(__('Found <b>1 p</b> in <b>%s</b> from <b>%s</b> to <b>%s</b>'), $params['address']);
                    }
                } else {
                    if (empty($params['address'])) {
                        $search_str = sprintf(__('Found <b>%s Properties</b>'), $total_result);
                    }

                    if (!empty($params['address'])) {
                        $search_str = sprintf(__('Found <b>%s Properties</b> in <b>%s</b> from <b>%s</b> to <b>%s</b>'), $total_result, $params['address']);
                    }
                }
            } else {
                $search_str = __('No Properties found');
            }
...
if (!function_exists('get_Property_permalink')) {
    function get_Property_permalink($post_slug)
    {
        return url("property/{$post_slug}");
    }
}
...
if (!function_exists('get_filter_status')) {
    function get_filter_status($post_type = 'post')
    {
        ...
        <div class="filter-action pl-2 mt-3">
            <a href="<?php echo dashboard_url($service_name . 'all-' . $post_type . '?' . $ext) ?>" class="<?php if ($status == '') {
            ...
            ?>
                    <a href="<?php echo dashboard_url($service_name . 'all-' . $post_type . '?status=' . $kpst . '&' . $ext) ?>" class="<?php echo $class; ?>">
                        <?php echo $vpst ?>
                    </a>
            <?php
                }
...
if (!function_exists('get_service_settings')) {
    function get_service_settings($service = GMZ_SERVICE_ROOM)
    {
        ...
        // // // Check if the service is GMZ_SERVICE_PROPERTY
        if ($service === 'property') {
            return [
                'general' => [
                    'id' => 'general_settings',
            ...
            // $data = Eventy::filter($service . '_fields', admin_config('settings', $service));

            // return Eventy::filter($service . '_fields', admin_config('settings', $service));
        }

        // $data = Eventy::filter('gmz_' . $service . '_fields', admin_config('settings', $service));

        // For other services, prepend 'gmz_' before the service name
