...
<section class="list-location py-40">
	<div class="container">
	<h4 class="mb-2 text-3xl font-medium text-center">Find your inspiration with Vil hive</h4>
	<p class="py-2 text-center text-gray-500 mb-2">Properties By Location </p>
					...
					if($number_services > 1){
						foreach ($all_services as $service){
							...
							elseif($service == GMZ_SERVICE_APARTMENT){
						        ...
						        $find_services[] = [
									'url' => $search_url,
									'text' => sprintf(_n(__('%s Properties'), __('%s Properties'), $number_service), $number_service)
								];
							}
						...
						if($all_services[0] == GMZ_SERVICE_PROPERTY){
					        $text = sprintf(_n(__('There are %s Properties'), __('There are %s Properties'), $number_service), $number_service);
					    }
