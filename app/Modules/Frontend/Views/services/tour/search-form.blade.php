...
<form id="search-tour" method="GET" class="search-form tour" action="{{url('tour-search')}}">
    <div class="search-form__basic">
        ...
        <div class="search-form__from date-group">
            ...
            <span class="check-in-render" data-date-format="{{get_date_format_moment()}}" style="color: black;">
                @if(!empty($checkIn))
        ...
        <div class="search-form__to date-group">
            <i class="fal fa-calendar-alt"></i>
            <span class="check-out-render" data-date-format="{{get_date_format_moment()}}" style="color: black;">
                @if(!empty($checkOut))
        ...
        <div class="search-form__guest tour">
            <div class="dropdown">
                <div class="dropdown-toggle" id="dropdownGuestButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    ...
                    <span class="guest-render" style="color: black;">
                        @php
            ...
            <div class="cont">
                <div class="row">
                    ...
                    @if(!empty($tour_types))
                        <div class="col-md-6 gmz-checkbox-wrapper">
                            ...
                            @foreach($tour_types as $key => $type)
                                <label class="checkbox-inline"><input type="checkbox" class="gmz-checkbox-item" name="tour_types[]" value="{{$key}}"><span style="color: black;">{{get_translate($type)}}</span></label>
                            @endforeach
                    ...
                    @if(!empty($tour_include))
                        <div class="col-md-6 gmz-checkbox-wrapper">
                            <div class="search-form__label" style="color: black; text-align:left; padding-left:10px;">{{__('Tour Includes')}}</div>
                            @foreach($tour_include as $key => $value)
                                <label class="checkbox-inline"><input type="checkbox" class="gmz-checkbox-item" name="tour_includes[]" value="{{$key}}"><span style="color: black;">{{get_translate($value)}}</span></label>
                            @endforeach
                    ...
                    @if(!empty($tour_exclude))
                        <div class="col-md-6 gmz-checkbox-wrapper">
                            <div class="search-form__label" style="color: black; text-align:left; padding-left:10px;">{{__('Tour Excludes')}}</div>
                            @foreach($tour_exclude as $key => $value)
                                <label class="checkbox-inline"><input type="checkbox" class="gmz-checkbox-item" name="tour_excludes[]" value="{{$key}}"><span style="color: black;">{{get_translate($value)}}</span></label>
                            @endforeach
