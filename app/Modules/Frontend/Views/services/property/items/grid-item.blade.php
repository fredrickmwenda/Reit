@php
    ...
    $type = get_term('id', $item->sale_type);
...
<div class="apartment-item apartment-item--grid" data-plugin="matchHeight">
    <div class="apartment-item__thumbnail">
        @php echo add_wishlist_box($item->id, GMZ_SERVICE_PROPERTY); @endphp
        <a href="{{get_property_permalink($item->post_slug)}}">
            <img src="{{$img}}" alt="{{$title}}">
        </a>
        @if(!empty($type))
            @php
                $search_url = add_query_arg(['sale_type' => $type->id], $search_url);
            @endphp
    ...
    <div class="apartment-item__details">
        ...
        @endif
        <h3 class="apartment-item__title"><a href="{{get_property_permalink($item->post_slug)}}">{{$title}}</a></h3>
        ...
        <div class="d-flex justify-content-between align-items-center">
            ...
            <a class="btn btn-primary apartment-item__view-detail" href="{{get_property_permalink($item->post_slug)}}">{{__('View Detail ')}}</a>
        </div>
    </div>
</div>