@php
    $can_review = is_user_can_review('', $post['id'], GMZ_SERVICE_PROPERTY);
    $comments = get_comment_list($post['id'], [
        'number' => get_comment_per_page(),
        'page' => request()->get('review_page', 1),
        'type' => GMZ_SERVICE_PROPERTY,
    ]);
@endphp

<div class="gmz-comment-list mt-4" id="review-section">
    <h3 class="comment-count">
        {{ _n(__('%s review for this Property'), __('%s reviews for this Property'), $comments->total()) }}
    </h3>
    @if(!$comments->isEmpty())
        @php
...
@if($can_review)
    <div class="post-comment parent-form" id="gmz-comment-section">
        <div class="comment-form-wrapper">
            <form action="{{ url('add-comment') }}" class="comment-form form-sm gmz-form-action form-add-post-comment"
                ...
                <input type="hidden" name="comment_type" value="{{GMZ_SERVICE_PROPERTY}}"/>
                <div class="row">
    ...
    @else 
    <h3> Sign in to Create a Review </h3>
@endif