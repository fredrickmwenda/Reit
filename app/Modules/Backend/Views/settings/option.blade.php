<div class="widget-content widget-content-area icon-pill" id="gmz-settings-page">
    ...
    <ul class="nav nav-pills mb-3 mt-3" id="icon-pills-tab" role="tablist">
        ...
        <li class="nav-item">
            <a class="nav-link @if($key == 0) active @endif" id="icon-pills-{{$item['id']}}-tab" data-toggle="pill" data-target="#gmz-{{$item['id']}}" href="javascript:void(0)" role="tab" aria-controls="icon-pills-{{$item['id']}}" aria-selected="@if($key == 0) true @else false @endif">
                {{$item['label']}}
            </a>
        </li>
        @endforeach
    ...
    <div class="tab-content" id="icon-pills-tabContent">
        @foreach($settings['sections'] as $key => $item)
        <div class="tab-pane fade show @if($key == 0) active @endif" id="gmz-{{$item['id']}}" role="tabpanel" aria-labelledby="icon-pills-{{$item['id']}}">
