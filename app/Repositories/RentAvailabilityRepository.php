...
use App\Models\RentAvailability;
use App\Models\Rent;

class RentAvailabilityRepository extends AbstractRepository
{
    ...
    {
        $this->model = new RentAvailability();
    }
    ...
    public function removeCalendarItem($postID, $checkIn)
    {
        ...
        if ($exists) {
            if ($exists['booked'] <= 0) {
                ...
                $rentModel = new Rent();
                $rentObject = $rentModel->query()->find($postID);
                $basePrice = $rentObject['base_price'];
                $exists->update([
                    'price' => $basePrice,
                    'is_base' => 1
                ]);
