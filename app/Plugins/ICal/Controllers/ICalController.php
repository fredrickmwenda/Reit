...
use App\Plugins\ICal\Models\Rent as ModelsRent;
use App\Plugins\ICal\Models\RentAvailability;
...
class ICalController extends Controller
{
    ...
    {
        require_once \ICalCore::inst()->pluginPath . '/vendor/autoload.php';
        $services = ['room', 'apartment', 'rent','space', 'tour'];
    ...
    private function _importICalRent()
    {
        $model = new ModelsRent();
        $avaiModel = new RentAvailability();
        $iCalData = $model->getIcalsData();
        if (!$iCalData->isEmpty()) {
            foreach ($iCalData as $item) {
                $postID = $item->id;
    ...
    private function _getICalRent($id)
    {
        $model = new ModelsRent();
        $checkExists = $model->query()->find($id);
        if ($checkExists) {
            $today = Carbon::now()->timestamp;
            $avalModel = new RentAvailability();
            $data = $avalModel->getUnavailableData($id, $today);
            $events = [];
            if (!$data->isEmpty()) {
                foreach ($data as $item) {
            ...
            $calendarComponent = $componentFactory->createCalendar($calendar);
            header('Content-Type: text/calendar; charset=utf-8');
            header('Content-Disposition: attachment; filename="rent' . time() . '.ics"');
            echo $calendarComponent;
            exit();
        }
        return response()->redirectTo('/');
